      ...
      <li class="flex justify-between gap-4 py-4">
        <div class="flex gap-4">
          <div class="size-8 rounded-full flex items-center justify-center"
            ...
            <x-forum.logo class="h-6 text-black" />
          </div>
          <div class="flex-auto">
        ...
        <div class="hidden sm:flex sm:flex-col sm:items-end">
          <p class="text-sm text-white">{{ $question->category->name }}</p>
          <p class="mt-1 text-xs text-gray-300">{{ $question->created_at->diffForHumans() }}</p>
        </div>
      </li>
